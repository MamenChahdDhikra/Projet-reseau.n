...
COLORS     = ['#4CAF50', '#FBC02D', '#FF8C00', '#90CAF9', '#9E9E9E']

 
print("Chargement des données Arkansas — 10 bandes...")
...
if os.path.exists('AR_datasets.pt'):                  
    ...
    raise FileNotFoundError(
        "AR_datasets.pt introuvable.\n"
        "Lance d'abord le script de préparation Arkansas."
    )
...
print(f"\nMCTNet Arkansas — {n_params:,} paramètres")
...
print(f"  MLP             : (batch,   5)")           
print(f"\n  Référence papier : OA={PAPER_RESULTS['OA']} "
      f"Kappa={PAPER_RESULTS['Kappa']} F1={PAPER_RESULTS['F1']}")
...
print("\n" + "="*75)
print("  ENTRAÎNEMENT MCTNet — Arkansas (10 bandes, sans covariables)")
...
for epoch in range(1, CONFIG['epochs'] + 1):
    ...
    if val_oa > best_val_oa:
        ...
        best_epoch     = epoch
        patience_count = 0
        torch.save(model.state_dict(), 'best_model_AR_bands.pt')
        marker = ' ✅'
    else:
        patience_count += 1
        marker = ''
...
model.load_state_dict(torch.load('best_model_AR_bands.pt', weights_only=True))
...
print(f"\nRésultats Arkansas — 10 bandes :")
print(f"  OA    : {test_oa:.4f}   (papier: {PAPER_RESULTS['OA']})")
print(f"  Kappa : {test_kappa:.4f}   (papier: {PAPER_RESULTS['Kappa']})")
print(f"  F1    : {test_f1:.4f}   (papier: {PAPER_RESULTS['F1']})")
...
fig.suptitle('Courbes — MCTNet sans covariables — Arkansas',
...
axes[1].plot(epochs_range, history['val_oa'],
             label='Val',   color='#F44336', linewidth=1.5)
axes[1].axhline(y=PAPER_RESULTS['OA'], color='green', linestyle='--',
                linewidth=1.5, label=f"Papier ({PAPER_RESULTS['OA']})")
...
axes[2].plot(epochs_range, history['val_f1'],
             label='Val',   color='#F44336', linewidth=1.5)
axes[2].axhline(y=PAPER_RESULTS['F1'], color='green', linestyle='--',
                linewidth=1.5, label=f"Papier ({PAPER_RESULTS['F1']})")
axes[2].set_title('F1 Score (macro)'); axes[2].set_xlabel('Époque')
axes[2].legend(); axes[2].grid(alpha=0.3)

plt.tight_layout()
plt.savefig('learning_curves_AR_bands.png', dpi=150, bbox_inches='tight')
print("\n✅ Sauvegardé : learning_curves_AR_bands.png")
plt.show()
 
cm_norm = cm.astype(float) / cm.sum(axis=1, keepdims=True)

fig, axes = plt.subplots(1, 2, figsize=(16, 6))
fig.suptitle('Matrice de Confusion — MCTNet Arkansas (10 bandes)\n'
...
ax2.set_xticks(range(N_CLS)); ax2.set_yticks(range(N_CLS))
...
plt.tight_layout()
plt.savefig('confusion_matrix_AR_bands.png', dpi=150, bbox_inches='tight')
print("✅ Sauvegardé : confusion_matrix_AR_bands.png")
plt.show()

 
fig, axes = plt.subplots(1, 3, figsize=(16, 5))
fig.suptitle('Métriques par classe — MCTNet Arkansas (10 bandes)',
             fontsize=13, fontweight='bold')
...
for ax, vals, title, subtitle in [
    ...
    ax.axhline(y=vals.mean(), color='red', linestyle='--',
               linewidth=1.5, label=f'Moyenne ({vals.mean():.3f})')
...
plt.tight_layout()
plt.savefig('metrics_per_class_AR_bands.png', dpi=150, bbox_inches='tight')
print("✅ Sauvegardé : metrics_per_class_AR_bands.png")
plt.show()

fig, axes = plt.subplots(1, 3, figsize=(14, 5))
fig.suptitle('Résumé Global — MCTNet Arkansas (10 bandes) vs Papier',
             fontsize=13, fontweight='bold')

for (name, val, paper_val), ax in zip(
    [('OA',       test_oa,    PAPER_RESULTS['OA']),
     ('Kappa',    test_kappa, PAPER_RESULTS['Kappa']),
     ('F1 macro', test_f1,    PAPER_RESULTS['F1'])], axes):
...
plt.tight_layout()
plt.savefig('global_summary_AR_bands.png', dpi=150, bbox_inches='tight')
print("✅ Sauvegardé : global_summary_AR_bands.png")
plt.show()
...
def plot_geo_maps(y_true, y_pred):
    ...
    df_all = pd.concat([
        pd.read_csv(csv_files['Zone 1'])[['pixel_id','class_name','lon','lat']],
        pd.read_csv(csv_files['Zone 2'])[['pixel_id','class_name','lon','lat']],
    ], ignore_index=True)
    ...
    df_test['zone']    = df_test['pixel_id'].apply(
        lambda x: 'Zone 1' if 'Z1' in str(x) else 'Zone 2'
    )
    ...
    fig.suptitle(
        'Arkansas — Vérité terrain / Prédiction / Erreurs (10 bandes)',
    ...
    plt.savefig('geo_maps_AR_bands.png', dpi=150, bbox_inches='tight')
    print("✅ Sauvegardé : geo_maps_AR_bands.png")
    plt.show()
