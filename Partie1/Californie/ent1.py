...
print("Chargement des données Californie — 10 bandes...")
...
print(f"\nMCTNet sans covariables — {n_params:,} paramètres")
...
print("\n" + "="*75)
print("  ENTRAÎNEMENT MCTNet — Californie (10 bandes, sans covariables)")
...
for epoch in range(1, CONFIG['epochs'] + 1):
    ...
    if val_oa > best_val_oa:
        ...
        best_epoch     = epoch
        patience_count = 0
        torch.save(model.state_dict(), 'best_model_bands.pt')  
        marker = ' ✅'
    else:
        patience_count += 1
        marker = ''
...
model.load_state_dict(torch.load('best_model_bands.pt', weights_only=True))
...
print(f"\nRésultats sans covariables (10 bandes) :")
...
fig.suptitle('Courbes — MCTNet sans covariables — Californie',
...
plt.tight_layout()
plt.savefig('learning_curves_bands.png', dpi=150, bbox_inches='tight')
print("\n✅ Sauvegardé : learning_curves_bands.png")
plt.show()
 
cm_norm = cm.astype(float) / cm.sum(axis=1, keepdims=True)

fig, axes = plt.subplots(1, 2, figsize=(16, 6))
fig.suptitle('Matrice de Confusion — MCTNet sans covariables\n'
...
plt.tight_layout()
plt.savefig('6_confusion_matrix_bands.png', dpi=150, bbox_inches='tight')
print(" Sauvegardé : 6_confusion_matrix_bands.png")
plt.show()
 
fig, axes = plt.subplots(1, 3, figsize=(16, 5))
fig.suptitle('Métriques par classe — MCTNet sans covariables',
             fontsize=13, fontweight='bold')
...
plt.tight_layout()
plt.savefig('7_metrics_per_class_bands.png', dpi=150, bbox_inches='tight')
print("✅ Sauvegardé : 7_metrics_per_class_bands.png")
plt.show()
 
fig, axes = plt.subplots(1, 3, figsize=(14, 5))
fig.suptitle('Résumé Global — MCTNet sans covariables vs Papier',
             fontsize=13, fontweight='bold')

for (name, val, paper_val), ax in zip(
    [('OA', test_oa, 0.8524),
     ('Kappa', test_kappa, 0.8194),
     ('F1 macro', test_f1, 0.8301)], axes):
...
def plot_geo_maps(y_true, y_pred):
    ...
    fig.suptitle(
        'Californie — Vérité terrain / Prédiction / Erreurs bandes',
