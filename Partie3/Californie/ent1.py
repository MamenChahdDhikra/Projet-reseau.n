...
COLORS  = ['#9467bd', '#2ca02c', '#17becf', '#d62728', '#ff7f0e', '#7f7f7f']
 
# Résultats papier Californie (référence — à mettre à jour si disponible)
PAPER = {'OA': 0.9260, 'Kappa': 0.9018, 'F1': 0.9025}
 
# ─────────────────────────────────────────────────────────────
#  CHARGEMENT
# ─────────────────────────────────────────────────────────────
PT_PATH   = 'datasets.pt'
SAVE_PATH = 'best_model_SE_CA.pt'
 
print(f"Chargement datasets.pt... (device: {CONFIG['device']})")
...
print(f"\nMCTNetSE California — {n_params:,} paramètres")
...
# ─────────────────────────────────────────────────────────────
print("\n" + "="*75)
print("  ENTRAÎNEMENT MCTNetSE — California (10 bandes S2, 6 classes)")
...
print(f"\n  F1 / Précision / Rappel par classe :")
print(f"  {'Classe':<14} {'F1':>7} {'Prec':>7} {'Recall':>7}")
print(f"  {'-'*37}")
for cls, f1, prec, rec in zip(CLASSES, f1_cls, prec_cls, rec_cls):
    print(f"  {cls:<14} {f1:7.4f} {prec:7.4f} {rec:7.4f}")
...
fig.suptitle('Courbes — MCTNetSE California (SE Spectral Attention)',
...
plt.tight_layout()
plt.savefig('learning_curves_SE_CA.png', dpi=150, bbox_inches='tight')
print("\n✅ learning_curves_SE_CA.png sauvegardé")
plt.show()
...
fig.suptitle(f'Matrice de Confusion — MCTNetSE California\n'
...
plt.tight_layout()
plt.savefig('confusion_SE_CA.png', dpi=150, bbox_inches='tight')
print("✅ confusion_SE_CA.png sauvegardé")
plt.show()
...
fig.suptitle('MCTNetSE vs Baseline (papier) — California',
             fontsize=13, fontweight='bold')
...
plt.tight_layout()
plt.savefig('comparison_SE_CA.png', dpi=150, bbox_inches='tight')
print("✅ comparison_SE_CA.png sauvegardé")
plt.show()
# ─────────────────────────────────────────────────────────────
#  MÉTRIQUES PAR CLASSE
# ─────────────────────────────────────────────────────────────
fig, axes = plt.subplots(1, 3, figsize=(16, 5))
fig.suptitle('Métriques par classe — MCTNetSE California',
             fontsize=13, fontweight='bold')
...
plt.tight_layout()
plt.savefig('metrics_per_class_SE_CA.png', dpi=150, bbox_inches='tight')
print("✅ Sauvegardé : metrics_per_class_SE_CA.png")
plt.show()
# ─────────────────────────────────────────────────────────────
...
def plot_geo_maps_SE_CA(y_true, y_pred):
    csv_files = {
        'Zone 1': 'df_zone1.csv',
        'Zone 2': 'df_zone2.csv',
    }
    ...
    fig.suptitle(
        'California — Vérité terrain / Prédiction / Erreurs MCTNetSE',
    ...
    plt.savefig('geo_maps_SE_CA.png', dpi=150, bbox_inches='tight')
    print("✅ Sauvegardé : geo_maps_SE_CA.png")
    plt.show()

plot_geo_maps_SE_CA(y_true, y_pred)