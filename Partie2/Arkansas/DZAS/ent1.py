...
from p import prepare_all_data_with_soil, CropDatasetCovAR
from model_ar import MCTNet
...
print("Chargement des données Arkansas avec sol...")
torch.serialization.add_safe_globals([CropDatasetCovAR])

if os.path.exists('AR_datasets_with_soil.pt'):
    datasets = torch.load('AR_datasets_with_soil.pt',
                          weights_only=False)
    print("✅ AR_datasets_with_soil.pt chargé !")
else:
...
print(f"\nMCTNet Arkansas + Sol — {n_params:,} paramètres")
...
for epoch in range(1, CONFIG['epochs'] + 1):
    ...
    if val_oa > best_val_oa:
        ...
        best_epoch     = epoch
        patience_count = 0
        torch.save(model.state_dict(), 'best_model_ar_sol.pt')
        marker = ' ✅'
    else:
        patience_count += 1
        marker = ''
...
model.load_state_dict(torch.load('best_model_ar_sol.pt', weights_only=True))
...
fig.suptitle('Courbes — MCTNet + Sol — Arkansas',
...
plt.tight_layout()
plt.savefig('learning_curves_ar_sol.png', dpi=150, bbox_inches='tight')
print("\n✅ Sauvegardé : learning_curves_ar_sol.png")
plt.show()
...
fig.suptitle('Matrice de Confusion — MCTNet + Sol — Arkansas\n'
...
plt.tight_layout()
plt.savefig('confusion_matrix_ar_sol.png', dpi=150, bbox_inches='tight')
print("✅ Sauvegardé : confusion_matrix_ar_sol.png")
plt.show()
 
fig, axes = plt.subplots(1, 3, figsize=(16, 5))
fig.suptitle('Métriques par classe — MCTNet + Sol — Arkansas',
             fontsize=13, fontweight='bold')
...
plt.tight_layout()
plt.savefig('7_metrics_per_class_ar_sol.png', dpi=150, bbox_inches='tight')
print("✅ Sauvegardé : 7_metrics_per_class_ar_sol.png")
plt.show()

fig, axes = plt.subplots(1, 3, figsize=(14, 5))
fig.suptitle('Résumé Global — MCTnet sol — Arkansas vs avec bandes seulement',
             fontsize=13, fontweight='bold')
...
print("✅ Sauvegardé : global_summary_AR_all.png")
plt.show()
...
def plot_geo_maps(y_true, y_pred):
    ...
    fig.suptitle(
        'Arkansas — Vérité terrain / Prédiction / Erreurs sol',
