...
from p import prepare_all_data_with_topo, CropDatasetTopoAR  
...
COLORS     = ['#4CAF50', '#FBC02D', '#FF8C00',
              '#90CAF9', '#9E9E9E']
 
print("Chargement des données Arkansas avec topo...")
torch.serialization.add_safe_globals([CropDatasetTopoAR])

if os.path.exists('AR_datasets_with_topo.pt'):
    datasets = torch.load('AR_datasets_with_topo.pt', weights_only=False)
    print("✅ AR_datasets_with_topo.pt chargé !")
else:
...
for epoch in range(1, CONFIG['epochs'] + 1):
    ...
    if val_oa > best_val_oa:
        ...
        best_epoch     = epoch
        patience_count = 0
        torch.save(model.state_dict(), 'best_model_topo_AR.pt')
        marker = ' ✅'
    else:
        patience_count += 1
        marker = ''
...
model.load_state_dict(torch.load('best_model_topo_AR.pt', weights_only=True))
...
fig.suptitle('Courbes — MCTNet + Topo — Arkansas',
...
plt.tight_layout()
plt.savefig('learning_curves_topo_AR.png', dpi=150, bbox_inches='tight')
print("\n✅ Sauvegardé : learning_curves_topo_AR.png")
plt.show()
 
cm_norm = cm.astype(float) / cm.sum(axis=1, keepdims=True)

fig, axes = plt.subplots(1, 2, figsize=(16, 6))
fig.suptitle('Matrice de Confusion — MCTNet + Topo — Arkansas\n'
...
plt.tight_layout()
plt.savefig('confusion_matrix_topo_AR.png', dpi=150, bbox_inches='tight')
print("✅ Sauvegardé : confusion_matrix_topo_AR.png")
plt.show()
 
fig, axes = plt.subplots(1, 3, figsize=(16, 5))
fig.suptitle('Métriques par classe — MCTNet + Topo — Arkansas',
             fontsize=13, fontweight='bold')
...
plt.tight_layout()
plt.savefig('metrics_per_class_topo_AR.png', dpi=150, bbox_inches='tight')
print("✅ Sauvegardé : metrics_per_class_topo_AR.png")
plt.show()
fig, axes = plt.subplots(1, 3, figsize=(14, 5))
fig.suptitle('Résumé Global — MCTnet topologie — Arkansas vs avec bandes seulement',
             fontsize=13, fontweight='bold')
...
def plot_geo_maps(y_true, y_pred):
    ...
    fig.suptitle(
        'Arkansas — Vérité terrain / Prédiction / Erreurs climat',
...
def plot_geo_maps(y_true, y_pred):
    ...
    fig, axes = plt.subplots(2, 3, figsize=(26, 18))
    fig.suptitle(
        'Arkansas — Vérité terrain / Prédiction / Erreurs topologie',
        fontsize=14, fontweight='bold', y=0.98
    )
