...
if __name__ == '__main__':
    print("=" * 55)
    print("  TEST MCTNet — Arkansas")
    print("=" * 55)
  
    print("\n[2] Avec topo — in_channels=12, n_classes=5")
    x    = torch.randn(32, 36, 12)
    mask = torch.zeros(32, 36)
    model = MCTNet(in_channels=12, n_classes=5,
                   n_head=5, n_stage=3, proj_channels=10)
    out   = model(x, mask)
    print(f"  Entrée : {x.shape}")
    print(f"  Sortie : {out.shape} "
          f"{'✅' if out.shape == torch.Size([32,5]) else '❌'}")
