...
def load_data_with_topo():
    print("\nChargement données Arkansas avec topo...")

    df_z1 = pd.read_csv('AR_df_zone1_with_topo.csv')
    df_z2 = pd.read_csv('AR_df_zone2_with_topo.csv')
...
def build_tensors_with_topo(df):
    N = len(df)
    print(f"\nConstruction tenseurs (N={N}, T=36, C=12)...")
 
    X = np.zeros((N, N_TIMES, N_BANDS + 2), dtype=np.float32)
    ...
    landforms_vals = df['landforms_norm'].values
    for t in range(N_TIMES):
        X[:, t, 10] = landforms_vals 
    elevation_vals = df['elevation_norm'].values
    for t in range(N_TIMES):
        X[:, t, 11] = elevation_vals
...
class CropDatasetTopoAR(Dataset):
...
def split_data(df, X, Y, mask):
    ...
    for name, idx in [('train', idx_train),
                      ('val',   idx_val),
                      ('test',  idx_test)]:
        datasets[name] = CropDatasetTopoAR(
            X[idx], Y[idx], mask[idx]
        )

    return datasets
 
def prepare_all_data_with_topo():

    df_all     = load_data_with_topo()
    X, Y, mask = build_tensors_with_topo(df_all)
    ...
    torch.save(datasets, 'AR_datasets_with_topo.pt')
    print("\n✅ AR_datasets_with_topo.pt sauvegardé !")

    return datasets
 
if __name__ == '__main__':
    print("="*55)
    print("  PRÉPARATION DONNÉES ARKANSAS AVEC TOPO")
    print("="*55)
    ...
    print(f"\nCovariables topo pixel 0 :")
    print(f"  landforms_norm  (T01) : {x[0][10]:.4f}")
    print(f"  elevation_norm  (T01) : {x[0][11]:.4f}")
    print(f"  landforms_norm  (T18) : {x[17][10]:.4f}")
    print(f"  elevation_norm  (T18) : {x[17][11]:.4f}")
