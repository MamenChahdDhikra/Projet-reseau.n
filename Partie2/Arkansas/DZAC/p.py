...
def load_data_with_clim():
    print("\nChargement données Arkansas avec covariables climat...")

    df_z1 = pd.read_csv('AR_df_zone1_with_clim.csv')
    df_z2 = pd.read_csv('AR_df_zone2_with_clim.csv')
...
class CropDatasetClimAR(Dataset):
...
def split_data(df, X, Y, mask):
    ...
    for name, idx in [('train', idx_train),
                      ('val',   idx_val),
                      ('test',  idx_test)]:
        datasets[name] = CropDatasetClimAR(
...
def prepare_all_data_with_clim():
    ...
    torch.save(datasets, 'AR_datasets_with_clim.pt')
    print("\n AR_datasets_with_clim.pt sauvegardé !")

    return datasets
 
if __name__ == '__main__':
    print("="*55)
    print("  PRÉPARATION DONNÉES ARKANSAS AVEC 3 COV. CLIMAT")
    print("="*55)
    ...
    print(f"  → valeurs différentes (climat dynamique)")
