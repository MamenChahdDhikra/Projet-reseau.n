...
from p import prepare_all_data_with_clim, CropDatasetClimAR
...
print("Chargement des données Arkansas avec climat...")
torch.serialization.add_safe_globals([CropDatasetClimAR])

if os.path.exists('AR_datasets_with_clim.pt'):
    datasets = torch.load('AR_datasets_with_clim.pt', weights_only=False)
    print("✅ AR_datasets_with_clim.pt chargé !")
else:
...
print(f"\nMCTNet Arkansas + Climat — {n_params:,} paramètres")
...
print("\n" + "="*75)
print("  ENTRAÎNEMENT MCTNet + Climat — Arkansas")
...
for epoch in range(1, CONFIG['epochs'] + 1):
    ...
    if val_oa > best_val_oa:
        ...
        best_epoch     = epoch
        patience_count = 0
        torch.save(model.state_dict(), 'best_model_clim_AR.pt')
        marker = ' ✅'
    else:
        patience_count += 1
        marker = ''
...
model.load_state_dict(torch.load('best_model_clim_AR.pt', weights_only=True))
...
print(f"\nRésultats Arkansas avec climat :")
...
fig.suptitle('Courbes — MCTNet + Climat — Arkansas',
...
plt.tight_layout()
plt.savefig('learning_curves_clim_AR.png', dpi=150, bbox_inches='tight')
print("\n✅ Sauvegardé : learning_curves_clim_AR.png")
plt.show()
 
cm_norm = cm.astype(float) / cm.sum(axis=1, keepdims=True)

fig, axes = plt.subplots(1, 2, figsize=(16, 6))
fig.suptitle('Matrice de Confusion — MCTNet + Climat — Arkansas\n'
...
plt.tight_layout()
plt.savefig('6_confusion_matrix_clim_AR.png', dpi=150, bbox_inches='tight')
print(" Sauvegardé : 6_confusion_matrix_clim_AR.png")
plt.show()

fig, axes = plt.subplots(1, 3, figsize=(16, 5))
fig.suptitle('Métriques par classe — MCTNet + Climat — Arkansas',
             fontsize=13, fontweight='bold')
...
plt.tight_layout()
plt.savefig('7_metrics_per_class_clim_AR.png', dpi=150, bbox_inches='tight')
print(" Sauvegardé : 7_metrics_per_class_clim_AR.png")
plt.show()
fig, axes = plt.subplots(1, 3, figsize=(14, 5))
fig.suptitle('Résumé Global — MCTnet climat — Arkansas vs avec bandes seulement',
             fontsize=13, fontweight='bold')
