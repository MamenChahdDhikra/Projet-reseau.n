...
from p import prepare_all_data, CropDatasetAll        
...
print("Chargement des données Arkansas — toutes covariables...")
torch.serialization.add_safe_globals([CropDatasetAll])

if os.path.exists('AR_datasets_all.pt'):
    datasets = torch.load('AR_datasets_all.pt', weights_only=False)
    print(" AR_datasets_all.pt chargé !")
else:
...
print(f"\nMCTNet Arkansas toutes covariables — {n_params:,} paramètres")
...
print("\n" + "="*75)
print("  ENTRAÎNEMENT MCTNet + Toutes Covariables — Arkansas")
print(f"  epochs={CONFIG['epochs']} | batch={CONFIG['batch_size']} | "
      f"lr={CONFIG['lr']} | patience={CONFIG['patience']}")
print(f"  in_channels=18 (10 bandes + 3 sol + 2 topo + 3 climat)")
...
for epoch in range(1, CONFIG['epochs'] + 1):
    ...
    if val_oa > best_val_oa:
        ...
        best_epoch     = epoch
        patience_count = 0
        torch.save(model.state_dict(), 'best_model_AR_all.pt')
        marker = ' ✅'
    else:
        patience_count += 1
        marker = ''
...
model.load_state_dict(torch.load('best_model_AR_all.pt', weights_only=True))
...
print(f"\nRésultats Arkansas toutes covariables :")
...
fig.suptitle('Courbes — MCTNet + Toutes Covariables — Arkansas',
...
plt.tight_layout()
plt.savefig('learning_curves_AR_all.png', dpi=150, bbox_inches='tight')
print("\n✅ Sauvegardé : learning_curves_AR_all.png")
plt.show()
 
cm_norm = cm.astype(float) / cm.sum(axis=1, keepdims=True)

fig, axes = plt.subplots(1, 2, figsize=(16, 6))
fig.suptitle('Matrice de Confusion — MCTNet + Toutes Covariables — Arkansas\n'
...
plt.tight_layout()
plt.savefig('confusion_matrix_AR_all.png', dpi=150, bbox_inches='tight')
print("✅ Sauvegardé : 6_confusion_matrix_AR_all.png")
plt.show()
 
fig, axes = plt.subplots(1, 3, figsize=(16, 5))
fig.suptitle('Métriques par classe — MCTNet + Toutes Covariables — Arkansas',
             fontsize=13, fontweight='bold')
...
plt.tight_layout()
plt.savefig('metrics_per_class_AR_all.png', dpi=150, bbox_inches='tight')
print("✅ Sauvegardé : 7_metrics_per_class_AR_all.png")
plt.show()
 
fig, axes = plt.subplots(1, 3, figsize=(14, 5))
fig.suptitle('Résumé Global — MCTNet + Toutes Covariables — Arkansas vs avec bandes seulement',
             fontsize=13, fontweight='bold')

metrics_data = [
    ('OA',       test_oa,    0.9693),
    ('Kappa',    test_kappa, 0.9533),
    ('F1 macro', test_f1,    0.9642),
]
for (name, val, paper_val), ax in zip(metrics_data, axes):
...
plt.tight_layout()
plt.savefig('global_summary_AR_all.png', dpi=150, bbox_inches='tight')
print("✅ Sauvegardé : 9_global_summary_AR_all.png")
plt.show()
...
def plot_geo_maps(y_true, y_pred):
    ...
    fig.suptitle(
        'Arkansas — Vérité terrain / Prédiction / Erreurs F3',
