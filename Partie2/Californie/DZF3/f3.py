...
def merge_zone(zone_name):
    ...
    df_clim = pd.read_csv(f'df_{zone_name}_with_clim.csv')
    df_soil = pd.read_csv(f'df_{zone_name}_with_soil.csv')
    df_topo = pd.read_csv(f'df_{zone_name}_with_topologie.csv')
    ...
    cov_cols = SOIL_COLS + ['elevation', 'landforms']
