...
print("Chargement des données Californie — toutes covariables...")
torch.serialization.add_safe_globals([CropDatasetAll])

if os.path.exists('datasets_all.pt'):
    datasets = torch.load('datasets_all.pt', weights_only=False)
    print("✅ datasets_all.pt chargé !")
else:
...
print(f"\nMCTNet toutes covariables — {n_params:,} paramètres")
...
print("\n" + "="*75)
print("  ENTRAÎNEMENT MCTNet + Toutes Covariables — Californie")
...
model.load_state_dict(torch.load('best_model_all.pt', weights_only=True))
...
fig.suptitle('Courbes — MCTNet + Toutes Covariables — Californie',
...
print("\n✅ Sauvegardé : learning_curves_all.png")
plt.show()
 
cm_norm = cm.astype(float) / cm.sum(axis=1, keepdims=True)

fig, axes = plt.subplots(1, 2, figsize=(16, 6))
fig.suptitle('Matrice de Confusion — MCTNet + Toutes Covariables\n'
...
plt.tight_layout()
plt.savefig('6_confusion_matrix_all.png', dpi=150, bbox_inches='tight')
print("✅ Sauvegardé : 6_confusion_matrix_all.png")
plt.show()
 
fig, axes = plt.subplots(1, 3, figsize=(16, 5))
fig.suptitle('Métriques par classe — MCTNet + Toutes Covariables',
             fontsize=13, fontweight='bold')
...
plt.tight_layout()
plt.savefig('7_metrics_per_class_all.png', dpi=150, bbox_inches='tight')
print("✅ Sauvegardé : 7_metrics_per_class_all.png")
plt.show()
 
fig, axes = plt.subplots(1, 3, figsize=(14, 5))
fig.suptitle('Résumé Global — MCTNet F3 vs avec bandes',
             fontsize=13, fontweight='bold')
...
def plot_geo_maps(y_true, y_pred):
    ...
    # ← pas besoin de recalculer 'zone' — déjà dans df_all
    ...
    fig.suptitle(
        'Californie — Vérité terrain / Prédiction / Erreurs F3',
