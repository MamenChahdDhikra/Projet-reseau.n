...
from p import prepare_all_data_with_soil, CropDatasetCov
...
print("Chargement des données Californie avec sol...")
torch.serialization.add_safe_globals([CropDatasetCov])

if os.path.exists('datasets_with_soil.pt'):
    datasets = torch.load('datasets_with_soil.pt', weights_only=False)
    print("✅ datasets_with_soil.pt chargé !")
else:
...
print(f"\nMCTNet avec sol — {n_params:,} paramètres")
...
print("\n" + "="*75)
print("  ENTRAÎNEMENT MCTNet + Sol — Californie")
...
for epoch in range(1, CONFIG['epochs'] + 1):
    ...
    if val_oa > best_val_oa:
        ...
        best_epoch     = epoch
        patience_count = 0
        torch.save(model.state_dict(), 'best_model_cov.pt')
        marker = ' ✅'
    else:
        patience_count += 1
        marker = ''
...
model.load_state_dict(torch.load('best_model_cov.pt', weights_only=True))
...
print(f"\nRésultats avec covariables sol :")
...
fig.suptitle('Courbes — MCTNet + Sol — Californie',
...
plt.tight_layout()
plt.savefig('learning_curves_cov.png', dpi=150, bbox_inches='tight')
print("\n✅ Sauvegardé : learning_curves_cov.png")
plt.show()
 
cm_norm = cm.astype(float) / cm.sum(axis=1, keepdims=True)

fig, axes = plt.subplots(1, 2, figsize=(16, 6))
fig.suptitle('Matrice de Confusion — MCTNet + Sol\n'
...
plt.tight_layout()
plt.savefig('6_confusion_matrix_cov.png', dpi=150, bbox_inches='tight')
print("✅ Sauvegardé : 6_confusion_matrix_cov.png")
plt.show()
 
fig, axes = plt.subplots(1, 3, figsize=(16, 5))
fig.suptitle('Métriques par classe — MCTNet + Sol',
             fontsize=13, fontweight='bold')
...
for ax, vals, title, subtitle in metrics_list:
    ...
    ax.axhline(y=ref, color='red', linestyle='--',
...
plt.tight_layout()
plt.savefig('7_metrics_per_class_cov.png', dpi=150, bbox_inches='tight')
print("✅ Sauvegardé : 7_metrics_per_class_cov.png")
plt.show()
 
fig, axes = plt.subplots(1, 3, figsize=(14, 5))
fig.suptitle('Résumé Global — MCTNet sol vs avec bandes',
             fontsize=13, fontweight='bold')
...
def plot_geo_maps(y_true, y_pred):
    ...
    fig.suptitle(
        'Californie — Vérité terrain / Prédiction / Erreurs sol',
