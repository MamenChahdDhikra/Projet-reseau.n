...
print("Chargement des données Californie avec topo...")
torch.serialization.add_safe_globals([CropDatasetCov])

if os.path.exists('datasets_with_topo.pt'):
    datasets = torch.load('datasets_with_topo.pt', weights_only=False)
...
print(f"\nMCTNet avec topo — {n_params:,} paramètres")
print(f"  Entrée          : (batch, 36, 12)")
print(f"  Projection 12→10: (batch, 36, 10)")
...
print("\n" + "="*75)
print("  ENTRAÎNEMENT MCTNet + Topo — Californie")
...
for epoch in range(1, CONFIG['epochs'] + 1):
    ...
    if val_oa > best_val_oa:
        ...
        best_epoch     = epoch
        patience_count = 0
        torch.save(model.state_dict(), 'best_model_topo.pt')
        marker = ' ✅'
    else:
        patience_count += 1
        marker = ''
...
model.load_state_dict(torch.load('best_model_topo.pt', weights_only=True))
...
print(f"\nRésultats avec covariables topo :")
...
fig.suptitle('Courbes — MCTNet + Topo — Californie',
...
plt.tight_layout()
plt.savefig('learning_curves_topo.png', dpi=150, bbox_inches='tight')
print("\n✅ Sauvegardé : learning_curves_topo.png")
plt.show()
...
plt.tight_layout()
plt.savefig('6_confusion_matrix_topo.png', dpi=150, bbox_inches='tight')
...
plt.tight_layout()
plt.savefig('7_metrics_per_class_topo.png', dpi=150, bbox_inches='tight')
print("✅ Sauvegardé : 7_metrics_per_class_topo.png")
plt.show()
 
fig, axes = plt.subplots(1, 3, figsize=(14, 5))
fig.suptitle('Résumé Global — MCTNet topologie vs avec bandes',
             fontsize=13, fontweight='bold')
...
def plot_geo_maps(y_true, y_pred):
    ...
    fig.suptitle(
        'Californie — Vérité terrain / Prédiction / Erreurs topologie',
