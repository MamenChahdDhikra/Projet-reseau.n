...
def load_data_with_topo():
    ...
    df_z1 = pd.read_csv('df_zone1_with_topologie.csv')
    df_z2 = pd.read_csv('df_zone2_with_topologie.csv')
...
def build_tensors_with_topo(df):
    ...
    landforms_vals = df['landforms'].values   
    for t in range(N_TIMES):
        X[:, t, 10] = landforms_vals             
 
    elevation_vals = df['elevation'].values 
    for t in range(N_TIMES):
...
def prepare_all_data_with_topo():
    ...
    torch.save(datasets, 'datasets_with_topo.pt')
    print("\n✅ datasets_with_topo.pt sauvegardé !")

    return datasets
 
if __name__ == '__main__':
    print("="*55)
    print("  PRÉPARATION DONNÉES AVEC 2 COVARIABLES SOL")
    print("="*55)
    ...
    print(f"  landforms   (T01) : {x[0][10]:.4f}")
    print(f"  elevation (T01) : {x[0][11]:.4f}")
   
    print(f"  landforms  (T18) : {x[17][10]:.4f}")
    print(f"  elevation (T18) : {x[17][11]:.4f}")
