...
def merge_zone_clim(zone_csv, zone_name, out_path):
    ...
    if n_missing > 0:
        print(f"  ⚠️  {n_missing} valeurs manquantes → remplissage par moyenne")
...
if __name__ == '__main__':
    ...
    df_z1 = merge_zone_clim(
        zone_csv  = 'df_zone1.csv',
        zone_name = 'CA_Zone1',
        out_path  = 'df_zone1_with_clim.csv'
    )

    df_z2 = merge_zone_clim(
        zone_csv  = 'df_zone2.csv',
        zone_name = 'CA_Zone2',
        out_path  = 'df_zone2_with_clim.csv'
    )

    print(f"\n{'='*50}")
    print(f"✅ FUSION TERMINÉE !")
    print(f"  df_zone1_with_clim.csv : {df_z1.shape}")
    print(f"  df_zone2_with_clim.csv : {df_z2.shape}")
